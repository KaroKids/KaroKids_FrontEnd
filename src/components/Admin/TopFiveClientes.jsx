      ...
      try {
        ...
        if (response.data) {
          ...
          response.data.forEach((item) => {
            countFive++;
            const nombreItem =
              item.usuario.nombre_usuario.charAt(0).toUpperCase() +
              item.usuario.nombre_usuario.slice(1) +
              " " +
              item.usuario.apellido_usuario.charAt(0).toUpperCase() +
              item.usuario.apellido_usuario.slice(1);
            const cantidad = Number(item.cantidad);

            if (countFive <= 5) formattedData.push([nombreItem, cantidad]);
          });
