...
import { getAllOrdenes, getOrdenesByName } from "@/redux/ordenesActions";
...
export default function Orders() {
  ...
  useEffect(() => {
    if (ordenes.length === 0) dispatch(getAllOrdenes());
    if (query.length > 0) {
      console.log(query);
  ...
  const handleStatusPayment = (ordenID, estatus) => {
    ...
    Swal.fire({
      title:
        estatus === "pendiente" ? "Cambiar a Aprobado" : "Cambiar a Pendiente",
      text:
        estatus === "pendiente"
            ...
            {ordenes &&
              ordenes.map((item) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">{item.Usuario.nombre_usuario}</td>
                  <td className="px-6 py-4">{numberMask(item.coste_total)}</td>
                  <td className="px-6 py-4">
                    {" "}
                    {item.estado_pago === "aprobado" ? (
                      <button
                        onClick={() => handleStatusPayment(item.orden_id, true)}
