...
const ProductCart = () => {
	...
	const handleIncrementarLS = async (e, id, talla, color, cantidad) => {
		e.preventDefault();
		//VALIDACION DE DISPONIBILIDAD DE STOCK
		const { payload } = await dispatch(getProductsById(id));
		const searchColor = payload.stock[talla].find(
			(el) => el.color == color.toLowerCase()
		);
		const availableStock = searchColor.cantidad;
		//VALIDACION DE DISPONIBILIDAD DE STOCK
		if (user.accessToken) {
